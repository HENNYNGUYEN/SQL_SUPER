...
--giá mua, giá bán, số lượng, trị giá mua, trị giá bán.
	...
	CREATE VIEW V22 AS
	SELECT DISTINCT HOADON.MAHD,HOADON.NGAY,TENKH,KHACHHANG.DIACHI,V21.TONGTG
	FROM HOADON,KHACHHANG,CTHD,v21
	WHERE HOADON.MAKH=KHACHHANG.MAKH AND CTHD.MAHD=HOADON.MAHD AND V21.MAHD=HOADON.MAHD
	AND V21.TONGTG=(SELECT MAX(TONGTG) FROM V21)
	--XEM DU LIEU
	SELECT  * FROM V22
	----23.Lấy ra hóa đơn có tổng trị giá lớn nhất trong tháng 5/2023 gồm các thông tin:
	--Số hóa đơn, ngày, tên khách hàng, địa chỉ khách hàng, tổng trị giá của hóa đơn.
	CREATE VIEW V23 AS
	SELECT HOADON.MAHD,HOADON.NGAY,TENKH,KHACHHANG.DIACHI,V21.TONGTG FROM HOADON,KHACHHANG,V21
	WHERE MONTH(HOADON.NGAY)=5 AND V21.MAHD=HOADON.MAHD AND HOADON.MAKH=KHACHHANG.MAKH 
	AND V21.TONGTG=(SELECT MAX(V21.TONGTG) FROM V21)
	-- XEM DU LIEU 
	SELECT * FROM V23 
	WHERE 
	...
	CREATE VIEW V25 AS
	SELECT DISTINCT MONTH(NGAY) AS [THÁNG] ,MAKH, COUNT(MAHD) AS [TỔNG HÓA ĐƠN] FROM HOADON GROUP BY MAKH,NGAY
	-- XEM DU LIEU 
	SELECT * FROM V25
	--26.	Lấy ra các thông tin của khách hàng có số lượng hóa đơn mua hàng nhiều nhất.
	CREATE VIEW V26 AS
	SELECT DISTINCT KHACHHANG.MAKH,TENKH,DT,DIACHI,V24.[TỔNG HÓA ĐƠN]  FROM HOADON ,KHACHHANG,V24 
	WHERE HOADON.MAKH=KHACHHANG.MAKH AND V24.MAKH=KHACHHANG.MAKH 
	AND V24.[TỔNG HÓA ĐƠN]=(SELECT MAX([TỔNG HÓA ĐƠN]) FROM V24)
	-- XEM DU LIEU 
	SELECT * FROM V26
	--27.	Lấy ra các thông tin của khách hàng có số lượng hàng mua nhiều nhất.
	CREATE VIEW V27 AS
	SELECT DISTINCT KHACHHANG.MAKH,TENKH,DT,DIACHI,SL FROM HOADON ,KHACHHANG,CTHD
	WHERE HOADON.MAKH=KHACHHANG.MAKH AND CTHD.MAHD=HOADON.MAHD AND KHACHHANG.MAKH=HOADON.MAKH
	AND SL=(SELECT MAX(SL) FROM CTHD)
	-- XEM DU LIEU 
	SELECT * FROM V27
	...
	CREATE VIEW V29 AS
	SELECT VATTU.MAVT, TENVT,DVT, GIAMUA, SLTON  FROM VATTU, CTHD WHERE VATTU.MAVT=CTHD.MAVT
	 AND CTHD.MAVT=(SELECT MAX(CTHD.MAVT) FROM CTHD)
	 -- XEM DU LIEU 
	SELECT * FROM V29
	...
	CASE WHEN (NOT EXISTS (SELECT MAKH FROM HOADON)) THEN 0
		 ELSE (SELECT COUNT(HOADON.MAHD) FROM HOADON
	WHERE KHACHHANG.MAKH=HOADON.MAKH
		GROUP BY HOADON.MAKH)
		END AS SLHOADONDAMUA
	FROM KHACHHANG
	-- XEM DU LIEU 
	SELECT * FROM V30
